#include "btAudio.h"
#include "PreferencesManager.h" // Include the header for the shared Preferences object
...
void btAudio::reconnect() {
  // Load remembered device address from flash
  ...
  switch (event) {
    case ESP_A2D_CONNECTION_STATE_EVT: {
      ...
      if (a2d->conn_stat.state == ESP_A2D_CONNECTION_STATE_CONNECTED) {
        ...
        // Turn on the LED when Bluetooth is connected
        digitalWrite(2, HIGH);
      } else if (a2d->conn_stat.state == ESP_A2D_CONNECTION_STATE_DISCONNECTED) {
        // Turn off the LED when Bluetooth is disconnected
        digitalWrite(2, LOW);
      }
      break;
    }
